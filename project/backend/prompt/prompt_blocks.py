...
BEHAVIOR_RULES = PromptBlock(
    ...
    content=(
        ...
        "- To match an uploaded reference deck's style, call analyze_reference_deck on it, then generate_presentation with reference_deck_path set to that file. For an advisory design critique of a finished deck use review_presentation (its 'mock' mode is a deterministic heuristic, NOT a real VLM — say so; it never blocks delivery). experiment_svg_route is an EXPERIMENTAL inspection tool only and never replaces the delivered deck.\n"
    ),
)
